...
// no-op. codec is the transport encryption (the same content key as the store's
...
// Run drives Sync on a ticker until ctx is cancelled. It is the must-have entry
// point: start it in a goroutine. A nil Syncer returns immediately. Tick
// failures are logged and retried on the next tick; they never stop the loop.
func (s *Syncer) Run(ctx context.Context) {
	...
	defer t.Stop()
	// An initial sync shortly after start, then on the ticker.
...
// halves are attempted even if one reports errors, so a transient failure in one
// direction does not block the other; the combined error (if any) is returned
...
func (s *Syncer) push(ctx context.Context) error {
	...
	for _, rec := range records {
		...
		plaintext, err := session.MarshalRecord(rec)
		if err != nil {
			s.log.Warn("serializing session for push", "id", rec.ID, "error", err)
			failures++
			continue
		}
		...
		if err != nil {
			s.log.Warn("encrypting session for push", "id", rec.ID, "error", err)
			failures++
			continue
		}
		blockHash := session.HashCiphertext(blob)
		if _, err := s.client.PutBlock(ctx, rec.ID, blockHash, blob); err != nil {
			s.log.Warn("pushing session block", "id", rec.ID, "error", err)
			failures++
			continue
		}
		s.mu.Lock()
...
// when peers push overlapping histories: a device that pushed a shorter version
// (and so holds a higher seq) must still absorb an older, longer block from
// another device. Union is commutative, so order does not matter.
...
func (s *Syncer) applyBlock(ctx context.Context, e ManifestEntry) error {
	...
	remote, err := session.UnmarshalRecord(plaintext)
	if err != nil {
		return err
	}
...
// markSynced records a record's current plaintext hash so the next push skips it
// (both sides have converged), preventing a push/pull ping-pong.
func (s *Syncer) markSynced(rec session.Record) {
	plaintext, err := session.MarshalRecord(rec)
	if err != nil {
		return
	}
