...
// algorithm change (e.g. XChaCha20-Poly1305 once golang.org/x/crypto is a
// dependency) can coexist and be selected by name on Load.
...
//
// AES-256-GCM (stdlib crypto/aes + crypto/cipher) is used rather than
// XChaCha20-Poly1305 because golang.org/x/crypto is not a dependency of this
...
// error when no key is configured (so callers can treat "no key" as "encryption
